...
if [ ! -f "$MEM0_DIR/project_aliases.json" ]; then
    ...
    echo "[3/5] 项目别名模板已创建（请编辑 ~/.mem0/project_aliases.json）"
else
    echo "[3/5] 项目别名已存在，跳过"
fi
...
if [ ! -f "$MEM0_DIR/config_local.json" ]; then
    ...
    echo "[4/5] 配置模板已创建（请编辑 ~/.mem0/config_local.json）"
else
    echo "[4/5] 配置已存在，跳过"
fi
...
if [ ! -f "$MEM0_DIR/.env" ]; then
    ...
    echo "[5/5] 环境变量模板已创建（可选编辑 ~/.mem0/.env）"
else
    echo "[5/5] .env 已存在，跳过"
fi
...
echo "  3. 确保Ollama运行中：ollama list"
